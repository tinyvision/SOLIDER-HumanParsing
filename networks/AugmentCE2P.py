...
InPlaceABNSync = lambda x: nn.Sequential(
                            nn.BatchNorm2d(x),
...
class Edge_Module(nn.Module):
    ...
    def __init__(self, in_fea=[96, 192, 384], mid_fea=192, out_fea=2):
...
class Decoder_Module(nn.Module):
    ...
    def __init__(self, num_classes, ch1=96, ch2=48, ch3=304):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(ch1, ch2, kernel_size=1, stride=1, padding=0, dilation=1, bias=False),
            InPlaceABNSync(ch2)
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(ch3, 256, kernel_size=1, padding=0, dilation=1, bias=False),
            InPlaceABNSync(256),
    ...
    if pretrained is not None:
        ...
        for i in saved_state_dict:
            ...
            if 'decoder' in i:
                continue
            if not i_parts[0] == 'fc':
                new_params['.'.join(i_parts[2:])] = saved_state_dict[i]

            if '.'.join(i_parts[2:]) in ['layer1.weight']:
                print(i, '->', '.'.join(i_parts[2:]))
...
def resnet101(num_classes=20, pretrained='./models/resnet101-imagenet.pth'):
    model = _ResNet(_Bottleneck, [3, 4, 23, 3], num_classes)
    settings = pretrained_settings['resnet101']['imagenet']
    initialize_pretrained_model(model, settings, pretrained)
    return model
...
def resnet50(num_classes=20, pretrained='./models/resnet50-imagenet.pth'):
    ...
    model= _ResNet(_Bottleneck, [3, 4, 6, 3], num_classes, fix_backbone=False)
    print(model)
